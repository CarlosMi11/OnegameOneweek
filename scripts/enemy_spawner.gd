...
func genEnemies(turno : int) -> Array[CharacterBody2D]:
	...
	var hay = false
	var index : Array[int] 
	for i in range(enemy_manager.enemiesToGenerate.size):
		if enemy_manager.enemiesToGenerate[i].get_meta("InitialTurn") == turno:
			hay = true
			index.append(i)
	var generados : Array[CharacterBody2D] = []
	if hay:
		for i in index:
			var multiplicador = enemy_manager.enemiesToGenerate[i].get_meta("SpawnMultiplier")
			for j in range(floor(turno * multiplicador)):
				var new_enemy : CharacterBody2D = enemy_manager.enemiesToGenerate[i].instantiate()
...
func startSpawn(array : Array[int]) -> Array[Node]:
	...
	for i in range(array.size()):
		for k in range(array[i]):
			var pos = global_position + Vector2(cos(randf_range(0, TAU)) * randf_range(0, radius), sin(randf_range(0, TAU)) * randf_range(0, radius))
			var new_enemy : Node = enemy_manager.enemyToGenerate[i].instantiate()
			new_enemy.global_position = pos
			new_enemy.z_index = 2
